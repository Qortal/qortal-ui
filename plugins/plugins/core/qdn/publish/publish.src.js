...
class PublishData extends LitElement {
				...
				<div class="layout horizontal center" style=" padding:12px 15px;">
					...
					<paper-card style="width:100%; max-width:740px;">
						<div style="margin:0; margin-top:20px;">
							<h3 style="margin:0; padding:8px 0; text-transform: capitalize; color: var(--black);">${translate("publishpage.pchange1")} / ${translate("publishpage.pchange2")} ${this.renderUploadCategory()}</h3>
	...
	renderUploadCategory() {
		const prefix1 = this.identifier
		const prefix2 = 'qortal_avatar'
		const prefix3 = 'qortal_group_avatar'

		if (this.category === 'Avatar') {
			if (prefix1.includes(prefix2)) {
				return html`Avatar`
			} else if (prefix1.includes(prefix3)) {
				return html`Group Avatar`
			}
		} else  {
			return html`${this.category}`
		}
	}
