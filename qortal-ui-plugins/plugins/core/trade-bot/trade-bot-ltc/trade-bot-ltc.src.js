...
class TradeBotLTC extends LitElement {
    static get properties() {
        return {
            ...
            tradesLtcQortal: { type: Array },
            doneTradesLtcQortal: { type: Array },
            tradesAvailableLtcQortal: { type: Array },
            tradeBotLtcQortal: { type: Array },
    ...
    constructor() {
        ...
        this.tradesLtcQortal = []
        this.doneTradesLtcQortal = []
        this.tradesAvailableLtcQortal = []
        this.tradeBotLtcQortal = []
		...
		<div class="my-historic-trades">
			<div class="box">
				...
				<div class="border-wrapper">
					...
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="myDoneTradesGrid" aria-label="My Buyed Qortal" .items="${this.doneTradesLtcQortal}">
						<vaadin-grid-column
							auto-width
							resizable
							header="${translate("tradepage.tchange11")}"
    ...
    tradeBotLTCTemplate() {
		...
		<div class="trade-bot-container">
			<div class="box-bot">
				...
				<div class="border-wrapper">
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="tradeBotLTCGrid" aria-label="Auto Buy Bot LTC" ?hidden="${this.isEmptyArray(this.tradeBotLtcBook)}" .items="${this.tradeBotLtcBook}">
						<vaadin-grid-column
							auto-width
							resizable
							header="${translate("tradepage.tchange8")} (QORT)"
    ...
    async firstUpdated() {
        ...
        const filterMyPriceTradesLTC = async () => {
            const myNode = window.parent.reduxStore.getState().app.nodeConfig.knownNodes[window.parent.reduxStore.getState().app.nodeConfig.node]
            const nodeUrl = myNode.protocol + '://' + myNode.domain + ':' + myNode.port
            const tradeBotLtcQortalUrl = `${nodeUrl}/crosschain/tradebot?foreignBlockchain=LITECOIN&apiKey=${this.getApiKey()}`
            const waitFor = ms => new Promise(res => setTimeout(res, ms))
            let timer

            const tradeBotLtcQortal = await fetch(tradeBotLtcQortalUrl).then(response => {
                return response.json()
            })

            this.tradeBotLtcQortal = tradeBotLtcQortal

            await waitFor(1000)

            if (this.isEmptyArray(this.tradeBotLtcBook) === true) {
                clearTimeout(timer)
                timer = setTimeout(filterMyPriceTradesLTC, 180000)
                return
            } else {
                clearTimeout(timer)
                timer = setTimeout(filterMyPriceTradesLTC, 180000)

                this.tradeBotAvailableLtcQortal = this.listedCoins.get(this.selectedCoin).openFilteredOrders.map(item => {
                ...
                if (this.isEmptyArray(this.tradeBotAvailableLtcQortal) === true) {
                    ...
                    this.checkAlice = this.tradeBotAvailableLtcQortal[0].qortalAtAddress
                }

                await waitFor(1000)

                if (this.tradeBotLtcQortal.some(item => item.atAddress === this.checkAlice)) {
                    return
                } else {
                    this.tradesAvailableLtcQortal = this.tradeBotAvailableLtcQortal
                }
            }

            await waitFor(1000)

            if (this.isEmptyArray(this.tradesAvailableLtcQortal) === true) {
                return
            } else {
                const botprice = this.round(parseFloat(this.tradeBotLtcBook[0].botLtcPrice))
                const changeamount = parseFloat(this.tradeBotLtcBook[0].botLtcQortAmount)
                const reduceamount = parseFloat(this.tradesAvailableLtcQortal[0].qortAmount)
                const tradeataddress = this.tradesAvailableLtcQortal[0].qortalAtAddress
                const newamount = this.round(parseFloat(changeamount - reduceamount))

                this.reAddAmount = this.round(parseFloat(this.tradeBotLtcBook[0].botLtcQortAmount))
                this.reAddPrice = this.round(parseFloat(this.tradeBotLtcBook[0].botLtcPrice))
                ...
                const newLtcTradebookItem = {
                    botLtcQortAmount: newamount,
                    botLtcPrice: botprice
                }
                ...
                if (this.isEmptyArray(this.tradeBotLtcBook) === true) {
                    ...
                    const checkBotFunds = this.round(parseFloat(this.tradeBotLtcBook[0].botLtcQortAmount) * parseFloat(this.tradeBotLtcBook[0].botLtcPrice))
                    const myBotFunds = this.round(parseFloat(this.listedCoins.get(this.selectedCoin).balance))
        ...
        const getDoneTradeLTC = async () => {
            ...
            this.doneTradesLtcQortal = doneLtcQortal.map(item => {
            ...
            setTimeout(getDoneTradeLTC, 600000)
        }
        ...
        parentEpml.ready().then(() => {
            parentEpml.subscribe('selected_address', async (selectedAddress) => {
                ...
                await this.ltcTradebook()  
            })

            parentEpml.subscribe('config', (c) => {
                if (!configLoaded) {
                    setTimeout(getQortLtcPrice, 1)
                    setTimeout(getDoneTradeLTC, 1)
                    configLoaded = true
                }
    ...
    async updateWalletBalance() {
        ...
        let _body = window.parent.reduxStore.getState().app.selectedAddress.ltcWallet.derivedMasterPublicKey
    ...
    */
    ...
    async buyAction() {
        ...
        let _foreignKey = this.selectedAddress.ltcWallet.derivedMasterPrivateKey
        ...
        const manageResponse = (response) => {
            if (response === true) {
                ...
                const newLtcTradebookItem = {
                    botLtcQortAmount: this.reAddAmount,
                    botLtcPrice: this.reAddPrice
                }
...
window.customElements.define('trade-bot-ltc', TradeBotLTC)