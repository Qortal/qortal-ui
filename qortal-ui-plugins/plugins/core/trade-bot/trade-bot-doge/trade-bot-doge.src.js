...
class TradeBotDOGE extends LitElement {
    static get properties() {
        return {
            ...
            tradesDogeQortal: { type: Array },
            doneTradesDogeQortal: { type: Array },
            tradesAvailableDogeQortal: { type: Array },
            tradeBotDogeQortal: { type: Array },
    ...
    constructor() {
        ...
        this.tradesDogeQortal = []
        this.doneTradesDogeQortal = []
        this.tradesAvailableDogeQortal = []
        this.tradeBotDogeQortal = []
		...
		<div class="my-historic-trades">
			<div class="box">
				...
				<div class="border-wrapper">
					...
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="myDoneTradesGrid" aria-label="My Buyed Qortal" .items="${this.doneTradesDogeQortal}">
						<vaadin-grid-column
							auto-width
							resizable
							header="${translate("tradepage.tchange11")}"
    ...
    tradeBotDOGETemplate() {
		...
		<div class="trade-bot-container">
			<div class="box-bot">
				...
				<div class="border-wrapper">
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="tradeBotDOGEGrid" aria-label="Auto Buy Bot DOGE" ?hidden="${this.isEmptyArray(this.tradeBotDogeBook)}" .items="${this.tradeBotDogeBook}">
						<vaadin-grid-column
							auto-width
							resizable
							header="${translate("tradepage.tchange8")} (QORT)"
    ...
    async firstUpdated() {
        ...
        const filterMyPriceTradesDOGE = async () => {
            const myNode = window.parent.reduxStore.getState().app.nodeConfig.knownNodes[window.parent.reduxStore.getState().app.nodeConfig.node]
            const nodeUrl = myNode.protocol + '://' + myNode.domain + ':' + myNode.port
            const tradeBotDogeQortalUrl = `${nodeUrl}/crosschain/tradebot?foreignBlockchain=DOGECOIN&apiKey=${this.getApiKey()}`
            const waitFor = ms => new Promise(res => setTimeout(res, ms))
            let timer

            const tradeBotDogeQortal = await fetch(tradeBotDogeQortalUrl).then(response => {
                return response.json()
            })

            this.tradeBotDogeQortal = tradeBotDogeQortal

            await waitFor(1000)

            if (this.isEmptyArray(this.tradeBotDogeBook) === true) {
                clearTimeout(timer)
                timer = setTimeout(filterMyPriceTradesDOGE, 180000)
                return
            } else {
                clearTimeout(timer)
                timer = setTimeout(filterMyPriceTradesDOGE, 180000)

                this.tradeBotAvailableDogeQortal = this.listedCoins.get(this.selectedCoin).openFilteredOrders.map(item => {
                ...
                if (this.isEmptyArray(this.tradeBotAvailableDogeQortal) === true) {
                    ...
                    this.checkAlice = this.tradeBotAvailableDogeQortal[0].qortalAtAddress
                }

                await waitFor(1000)

                if (this.tradeBotDogeQortal.some(item => item.atAddress === this.checkAlice)) {
                    return
                } else {
                    this.tradesAvailableDogeQortal = this.tradeBotAvailableDogeQortal
                }
            }

            await waitFor(1000)

            if (this.isEmptyArray(this.tradesAvailableDogeQortal) === true) {
                return
            } else {
                const botprice = this.round(parseFloat(this.tradeBotDogeBook[0].botDogePrice))
                const changeamount = parseFloat(this.tradeBotDogeBook[0].botDogeQortAmount)
                const reduceamount = parseFloat(this.tradesAvailableDogeQortal[0].qortAmount)
                const tradeataddress = this.tradesAvailableDogeQortal[0].qortalAtAddress
                const newamount = this.round(parseFloat(changeamount - reduceamount))

                this.reAddAmount = this.round(parseFloat(this.tradeBotDogeBook[0].botDogeQortAmount))
                this.reAddPrice = this.round(parseFloat(this.tradeBotDogeBook[0].botDogePrice))
                ...
                const newDogeTradebookItem = {
                    botDogeQortAmount: newamount,
                    botDogePrice: botprice
                }
                ...
                if (this.isEmptyArray(this.tradeBotDogeBook) === true) {
                    ...
                    const checkBotFunds = this.round(parseFloat(this.tradeBotDogeBook[0].botDogeQortAmount) * parseFloat(this.tradeBotDogeBook[0].botDogePrice))
                    const myBotFunds = this.round(parseFloat(this.listedCoins.get(this.selectedCoin).balance))
        ...
        const getDoneTradeDOGE = async () => {
            ...
            this.doneTradesDogeQortal = doneDogeQortal.map(item => {
            ...
            setTimeout(getDoneTradeDOGE, 600000)
        }
        ...
        parentEpml.ready().then(() => {
            parentEpml.subscribe('selected_address', async (selectedAddress) => {
                ...
                this.dogeWwallet = window.parent.reduxStore.getState().app.selectedAddress.dogeWallet.address
                await this.updateAccountBalance()
                await this.dogeTradebook()  
            })

            parentEpml.subscribe('config', (c) => {
                if (!configLoaded) {
                    setTimeout(getQortDogePrice, 1)
                    setTimeout(getDoneTradeDOGE, 1)
                    configLoaded = true
                }
    ...
    async updateWalletBalance() {
        ...
        let _body = window.parent.reduxStore.getState().app.selectedAddress.dogeWallet.derivedMasterPublicKey
    ...
    */
    ...
    async buyAction() {
        ...
        let _foreignKey = this.selectedAddress.dogeWallet.derivedMasterPrivateKey
        ...
        const manageResponse = (response) => {
            if (response === true) {
                ...
                const newDogeTradebookItem = {
                    botDogeQortAmount: this.reAddAmount,
                    botDogePrice: this.reAddPrice
                }
...
window.customElements.define('trade-bot-doge', TradeBotDOGE)